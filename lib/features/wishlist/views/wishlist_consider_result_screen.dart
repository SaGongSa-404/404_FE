...
class WishlistConsiderResultScreen extends ConsumerWidget {
  ...
  String _subtitle() {
    switch (caseType) {
      case ConsiderCaseType.caseA:
        return '고민한 만큼 값어치 있을 거예요\n너구리도 인정하는 현명한 소비예요';
      case ConsiderCaseType.caseB:
        return '솜사탕이 비에 녹아버렸어요\n다음 솜사탕은 너굴이와 함께 꼭 지켜봐요';
      case ConsiderCaseType.caseC:
        return '오늘 햇살이 따뜻하네요\n신중한 선택이 쌓이면 더 큰 기쁨이 돼요';
      case ConsiderCaseType.caseD:
        return '너무 기뻐서 날아갈 것 같아요\n유혹을 이겨낸 오늘, 스스로를 칭찬해요';
    }
  }

  String? _footerMessage() {
    return switch (caseType) {
      ConsiderCaseType.caseA || ConsiderCaseType.caseB =>
        '7일 후 만족도를 물어볼게요',
      ConsiderCaseType.caseC || ConsiderCaseType.caseD => null,
    };
  }
        ...
        child: Column(
          children: [
            ...
            Padding(
              ...
              child: Column(
                children: [
                  if (_footerMessage() case final message?) ...[
                    Text(
                      message,
                      style: TextStyle(
                        fontSize: 16 * scale,
